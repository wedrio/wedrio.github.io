...
- web前端
---
&emsp;&emsp;前面空了一部分没有做笔记，太懒了，后面补出来。
## strong与b、em与i？  
&emsp;&emsp;表现形态都是文本加粗和斜体  
&emsp;&emsp;strong和em是具备语义化的，而b和i是不具备语义化的。
<!--more-->
## 引用标签
&emsp;&emsp;blockquote：引用大段的段落解释  
&emsp;&emsp;q：引用小段的短语解释  
&emsp;&emsp;abbr：缩写或首字母缩略词  
&emsp;&emsp;address：引用文档地址信息  
&emsp;&emsp;cite：引用著作的标题   

## iframe嵌套页面
&emsp;&emsp;iframe元素会创建包含另外一个文档的内联框架（即行内框架）。  
也就是在一个页面中可以嵌入另外一个页面  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/html+css_zs10.jpg.png)
&emsp;&emsp;应用场景：数据传输、共享代码、局部刷新、第三方介入。  

## br与wbr
&emsp;&emsp;br换行、wbr软换行  

## pre与code
&emsp;&emsp;针对网页中的程序代码的显示效果  
## map与area
&emsp;&emsp;给特殊图形添加链接，area能添加的热区图形有：矩形，圆形，多边形  

## embed与object
&emsp;&emsp;嵌入一些多媒体，如flash动画、插件等，区别是兼容性  

## audio与video（H5的功能）
&emsp;&emsp;Audio嵌入音频文件  
&emsp;&emsp;Video嵌入视频文件  
&emsp;&emsp;默认控件不显示，可通过controls属性来显示控件。  
&emsp;&emsp;Source备选方案，看网页是否支持格式  

## 文字注解与文字方向
&emsp;&emsp;Ruby标签定义ruby注释（中文注音或字符），rt标签定义字符（中文注音或字符）的解释或发音。  
&emsp;&emsp;Bdo文字反向

## 扩展link标签
&emsp;&emsp;可以引入顶部标签小图标
## 扩展meta标签
&emsp;&emsp;Meta添加一些辅助信息

## html5新语义化标签（常用）
&emsp;&emsp;header 页眉  
&emsp;&emsp;footer  页脚  
&emsp;&emsp;main  主体  
&emsp;&emsp;hgroup  标题组合  
&emsp;&emsp;nav  导航  
&emsp;&emsp;article  独立的内容  
&emsp;&emsp;aside 辅助信息的内容  
&emsp;&emsp;section  区域  
&emsp;&emsp;figure  描述图像或视频  
&emsp;&emsp;figcaption 描述图像或视频的标题部分  
&emsp;&emsp;datalist  选项列表  
&emsp;&emsp;details/summary  文档细节/文档标题  
&emsp;&emsp;progress/meter  定义进度条/定义度量范围  
&emsp;&emsp;time  定义日期或时间  
&emsp;&emsp;mark 带有记号的文本  

## 表格扩展
&emsp;&emsp;添加单线： border-collapse：collapse  
&emsp;&emsp;隐藏空单元  empty-cells：hide  
&emsp;&emsp;斜线分类  border/rotate  
&emsp;&emsp;列分组  colgroup/col  
## 表单控件扩展
...
## grid网格布局
&emsp;&emsp;一个二维布局的方法,横纵两个方向总是同时存在。  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/grid.png) 
### grid-template-columns和grid-template-rows
&emsp;&emsp;对网格进行横纵划分，形成二维布局。单位可以是像素，百分比，自适应以及fr单位（网格剩余空间比例单位）。  
&emsp;&emsp;有时候，我们网格的划分是很规律的，如果要添加多个横纵网格时，可以利用repeat()语法进行简化操作。
### grid-template-areas和grid-template
&emsp;&emsp;area是区域的意思，grid-template-areas就是给我们的网格划分区域的，此时grid子项只要使用grid-area属性指定其隶属于哪个区。  
&emsp;&emsp;grid-template是grid-template-rows,grid-template-columns和grid-template-areas的属性的缩写。
### grid-column-gap和grid-row-gap
&emsp;&emsp;grid-column-gap和grid-row-gap属性用来定义网格中网格间隙的尺寸。  
&emsp;&emsp;grid-gap是grid-column-gap与grid-row-gap属性的缩写。
### justify-items和align-items
&emsp;&emsp;justify-items指定了网格元素的水平呈现方式，是水平拉伸显示，还是左中右对齐。align-items指定了网格元素的垂直呈现方式，是垂直拉伸显示，还是上中下对齐。  
&emsp;&emsp;place-items可以让align-items和justify-items属性写在单个声明中。  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/place-items.png)
### justify-content和algin-content
&emsp;&emsp;justify-content指定了网格元素的水平分布方式。algin-content指定了网格元素的垂直分布方式。place-content可以让align-content和justify-content属性写在一个css声明中。  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/justify-content.png)
### 作用在grid子项上的css属性
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/grid-child.png)
## 移动端之viewport视口
&emsp;&emsp;在移动端viewport视口就是浏览器显示页面内容的屏幕区域。在viewport中有两种视口，分别表示为，visual viewport（可视视口）和layout viewport（布局视口）。  
&emsp;&emsp;visual viewport固定大小跟屏幕相同，在上面，而layout viewport可改变大小，在下面。layout viewport默认大小为980像素，可通过document.documentElement.clientWidth获取。  
&emsp;&emsp;现代网页制作需要将layout viewport设置成跟visual viewport等同大小，方便进行网页制作。  
通过meta标签进行设置，name属性指定viewport值，content属性进行视口配置。  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/viewport_01.png)
## 流式布局
流失布局原则  
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/flowLayout.png)  
好处：大屏幕下显示更多内容。  
坏处：宽屏下比例会有以下不协调。
## rem布局
&emsp;&emsp;单位：  
&emsp;&emsp;&emsp;&emsp;em：是一个相对单位，1em等于当前元素或父元素的font-size值。  
&emsp;&emsp;&emsp;&emsp;rem：是一个相对单位，1rem等于根元素的font-size值。  
&emsp;&emsp;&emsp;&emsp;vw/vh：把屏幕分成100份，1vm等于屏幕宽的1%。
## 动态设置fontsize
&emsp;&emsp;通过js  
...
## 响应式布局
&emsp;&emsp;利用媒体查询，即media queries，可以针对不同的媒体类型定义不同的样式，从而实现响应式布局。  
媒体类型
![](https://cdn.jsdelivr.net/gh/wedrio/ImageHosting/wedrio-pic/media1.png)
常见选项：  
&emsp;&emsp;and、not  
&emsp;&emsp;min-width、max-width  
&emsp;&emsp;orientation：protrait、orientation：landscape  
&emsp;&emsp;link  
常见修改样式：  
&emsp;&emsp;display  
&emsp;&emsp;float  
&emsp;&emsp;width  
&emsp;&emsp;注：响应式代码写到要适配的CSS后面